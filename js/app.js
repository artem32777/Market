(() => {
    ...
    (() => {
        ...
        function productFlyAdd() {
            if (document.querySelector(".add-cart")) document.addEventListener("DOMContentLoaded", (() => {
                const cart = document.querySelector(".mini-cart__qty");
                const speedAnimation = 1e3;
                document.addEventListener("click", (e => {
                    const targetElement = e.target;
                    if (targetElement.closest(".add-cart")) {
                        const productSlide = targetElement.closest(".product");
                        const productImage = productSlide.querySelector(".swiper-slide-active");
        ...
        function spollers() {
            ...
            if (spollersArray.length > 0) {
                ...
                if (spollersRegular.length) if (document.documentElement.classList.contains("touch")) initSpollers(spollersRegular);
        ...
        function initSliders() {
            ...
            if (document.querySelector(".category-slider__slider")) new core(".category-slider__slider", {
        ...
        new lazyload_min({
            ...
            callback_enter: function(swiperElement) {
                new core("#" + swiperElement.id, swipersLazy);
            }
        });
        ...
        catalogOpen();
        if (document.querySelector(".btn-glow")) {
            document.documentElement.addEventListener("mouseover", btnGlowEffect);
            document.documentElement.addEventListener("mouseout", btnGlowEffect);
            document.documentElement.addEventListener("mousemove", btnGlowEffect);
            let bGlow, bGlowColor, bGlowSize;
            function btnGlowEffect(e) {
                const btn = e.target.closest(".btn-glow");
                if (!btn) return;
                if ("mouseover" === e.type) {
                    btn.insertAdjacentHTML("beforeend", `\n        <span class="btn-glow__glow"><span class="btn-glow__color"></span></span>\n      `);
                    bGlow = btn.querySelector(".btn-glow__glow");
                    bGlowColor = btn.querySelector(".btn-glow__color");
                    bGlowSize = Math.min(btn.offsetWidth, btn.offsetHeight);
                    bGlow.style.width = bGlow.style.height = `${bGlowSize}px`;
                    bGlowColor.style.width = `${btn.offsetWidth}px`;
                    bGlowColor.style.height = `${btn.offsetHeight}px`;
                }
                if ("mouseout" === e.type) btn.querySelector(".btn-glow__glow").remove();
                if ("mousemove" === e.type) {
                    const posX = e.pageX - (btn.getBoundingClientRect().left - scrollX);
                    const posY = e.pageY - (btn.getBoundingClientRect().top - scrollY);
                    bGlow.style.left = `${posX - bGlowSize / 2}px`;
                    bGlow.style.top = `${posY - bGlowSize / 2}px`;
                    bGlowColor.style.transform = `\n        translate(${posX - (btn.offsetWidth - bGlowSize / 2)}px,\n                  ${posY - (btn.offsetHeight - bGlowSize / 2)}px)`;
                }
            }
        }
        window["FLS"] = false;
